...
async def parse_syllabus(session_id: str, file: UploadFile = File(None), raw_text: str = ""):
    # Step 1 — extract text
    if file:
    ...
    # Step 3 — use Claude to extract structured topic list (minimal call)
...
Syllabus:
{raw_text[:3000]}"""  # cap tokens

    response = anthropic.messages.create(
        model="claude-haiku-4-5-20251001",  # cheapest model for structured parsing
        max_tokens=1000,
    ...
    try:
        ...
        raise HTTPException(500, "Claude returned unparseable JSON. Try again.")
...
{focus_text}"""
        ...
        try:
            recommended_focus = json.loads(resp.content[0].text)
        except Exception:
